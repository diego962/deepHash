...
from dph.config import hashs, osinfo
from dph.core.fileinfo import is_malware
from dph.log import log

log = log.get_logger()

rules = yara.compile(
        filepath=osinfo.which_dirname(__file__) +
...
def run():
    ...
    if arguments.filename:
        (hash_file, matches, is_malicious) = calculate_hash(
            arguments.filename, typeHash
        )
...
def deep_hash(directory, typehash=""):
    files = (
             osinfo.ls_dir(directory)
             if (osinfo.is_dir(directory))
             else [directory]
            )
